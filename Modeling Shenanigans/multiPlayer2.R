...
multiPred <- function(n.iter, playerNames, this_week, dotw = c("Monday", "Tuesday", "Wednesday", 
  ...
  hSigma <- matrix(1, nrow = n.iter + burnIn + 1, ncol = length(matchedNames))
  ...
  for (j in 2:(n.iter + burnIn + 1)) {
    ...
    for (i in nameSequence) {
      ...
      ## Hierarchical Variance
      hSigma[j, i] <- 1/rgamma(1, gamesPlayed[i]/2, hierarchicalSS[i]/2)
      sigmaMat[j, i] <- 1/rgamma(1, gamesPlayed[i]/2 + alphas[i], 
                                 allSS[i]/2 + betas[i])
      currentPrecision <- (1/sigmaMat[j, i] + 1/hSigma[j, i])^(-1)
      currentMean <- (priorMean[[i]]/hSigma[j, i] + sleeperPoints[[i]]/sigmaMat[j, i]) * currentPrecision
      thetaList[[i]][j, ] <- mvtnorm::rmvnorm(1, currentMean, diag(currentPrecision, gamesPlayed[i]))
      
      ## Posterior Predictive Sampling
      newThetas <- mvtnorm::rmvnorm(1, sleepEsts[[i]], diag(hSigma[j, i], totalEsts[i]))
