...
df_2024 %>% filter(name == "Scottie Barnes") %>% pull(sleeper_points) %>% rev %>% 
  plot(type = "l", xlab = "Games Played", ylab = "Sleeper Points", 
       main = "Scottie Barnes Sleeper Score: 2023-2024 Season") # This one too. Some players are definitely not stationary
segments(0, 30, 30, 30, col = "red", lwd = 2, lty = 2)
segments(30, 25, 61, 25, col = "blue", lwd = 2, lty = 2)
legend(3, 15, c("Running Average:  Games 1-30",
                 "Running Average: Games 31-61"), 
       col = c("red", "blue"), lwd = 2, lty = 2, cex = 0.65)
